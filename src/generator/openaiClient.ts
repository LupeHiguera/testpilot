      ...
      input: [
        {
          ...
          content: [
            ...
            "- For copy or selector drift, replace the brittle locator with a resilient one. When a control's label changed (a button OR a link), use a role locator whose name matches BOTH the old and new text via a regex, e.g.",
            ...
            '- When `observation` is present it is a FRESH read of the page at repair time (its current url, buttons, links, and inputs). Prefer locators that match the controls actually present in that observation, so the repair is grounded in the live UI rather than the stale failure.',
